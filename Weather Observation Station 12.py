...
# MAGIC %md
# MAGIC Query the list of CITY names from STATION that do not start with vowels and do not end with vowels. Your result cannot contain duplicates.
# MAGIC
# MAGIC Input Format
# MAGIC
...
# MAGIC FROM Station
# MAGIC WHERE REGEXP_LIKE(City, '^[^AEIOU].*[^aeiou]$');

# COMMAND ----------

from pyspark.sql.functions import col, substring, lower

result = df.withColumn("Isvowels",col("CITY").rlike("^[^AEIOU].*[^aeiou]$"))


result.where('Isvowels').distinct().show()