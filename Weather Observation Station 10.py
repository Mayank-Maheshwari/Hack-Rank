...
# MAGIC %md
# MAGIC Query the list of CITY names from STATION that do not end with vowels. Your result cannot contain duplicates.
# MAGIC
# MAGIC Input Format
# MAGIC
...
# MAGIC FROM Station
# MAGIC WHERE LOWER(RIGHT(City,1)) not in ('a','e','i','o','u') 

# COMMAND ----------

from pyspark.sql.functions import col, substring, lower

result = df.withColumn("last_letter",lower(substring(df.City,-1,1)))
