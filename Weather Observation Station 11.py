...
# MAGIC %md
# MAGIC Query the list of CITY names from STATION that either do not start with vowels or do not end with vowels. Your result cannot contain duplicates.
# MAGIC
# MAGIC Input Format
# MAGIC
...
# MAGIC FROM Station
# MAGIC WHERE LOWER(LEFT(City,1)) not in ('a','e','i','o','u') and LOWER(RIGHT(City,1)) not in ('a','e','i','o','u')
...
result = result.where("last_letter not in ('a','e','i','o','u')")
